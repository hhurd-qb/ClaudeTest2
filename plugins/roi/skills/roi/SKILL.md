---
name: roi
description: "- Trigger this skill whenever the user says \"/roi\" — with or without anything else in the message — no need to match any other phrasing once \"/roi\" appears. Scope a proposed AI/automation use case into a realistic, defensible ROI estimate and, on confirmation, file it to the Confluence ROI register and/or a Jira ticket for prioritization. Arithmetic is done by a deterministic engine (references/roi\\_engine.py) against a versioned, owned assumptions library (references/assumptions.json) — the house numbers are the single source of truth, so the same inputs always yield the same number for everyone. Runs a short interview in plain language, records any deviation from house assumptions as an explicit override with a reason, runs the engine, and always asks before publishing anywhere. On every trigger, also hands the person a standalone HTML intake form (references/intake-form.html) that collects a case's facts into the same schema — it does not calculate anything itself; all math still happens in this conversation via the real engine. Also use WHENEVER a user wants to estimate, scope, justify, or prioritize an AI or automation idea without typing \"/roi\" - 'ROI of automating X', 'should we build an agent for Y', 'scope this use case', 'add this to the ROI register', 'business case for [AI idea]'. Do NOT trigger for a generic ROI or business-case question with no AI/automation angle (e.g. 'ROI of a new office lease'), and do NOT trigger when a use case is pasted purely for reference or discussion rather than to be scored (e.g. 'here's a case someone else already wrote up, what do you think of it') - ask which the person wants if it's unclear. Requires code execution enabled to run the engine. No connector is required to produce an estimate — connected systems (Snowflake/Salesforce/Gainsight) are an optional trust upgrade when available, never a dependency; Atlassian is only needed for the final Confluence or Jira publish step."
---

# /roi — AI Use Case ROI

When the user types `/roi` — alone, or followed by a one-liner like `/roi automate QBR decks for CSMs` — start this workflow immediately from Step 0 below. Don't wait for a fuller description;  
...
outputs folder and present it via present\_files. Tell the person briefly what it is — a  
...
| --- | --- | --- |
| "What's this idea in one sentence — what does the AI actually do?" | "State the use\_case.one\_liner" | `use_case.one_liner` |
...
| "How many times a year does this happen? (e.g. 200 QBRs/year, 5,000 tickets/month)" | "eligible\_volume\_year?" | `labor.eligible_volume_year` |
| "Today, without AI, how long does one of these take from start to finish?" | "hours\_per\_instance?" | `labor.hours_per_instance` |
...
| *(revenue/redeploy only)* "What's a typical account worth per year — ARR or margin?" | "value\_per\_account?" | `revenue_chain.value_per_account` |
...
Record provenance for each key input in the inputs JSON `data_grounding` block with `source: "system"` (you verified it against something this session) or `source: "user"` (it's their word).  
